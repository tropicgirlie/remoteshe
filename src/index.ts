...
app.get('/api/jobs', async (c) => {
  let jobs = await c.env.KV.get<Job[]>('cached_jobs', 'json')
  if (!jobs) {
    jobs = await syncAllJobs()
    await c.env.KV.put('cached_jobs', JSON.stringify(jobs), {
...
app.get('/api/jobs/:id', async (c) => {
  const jobs = (await c.env.KV.get<Job[]>('cached_jobs', 'json')) ?? []
...
app.get('/api/health', async (c) => {
  const cached = await c.env.KV.get('cached_jobs')
...
export default {
  ...
  async scheduled(_event: ScheduledEvent, env: Bindings): Promise<void> {
    const jobs = await syncAllJobs()
    await env.KV.put('cached_jobs', JSON.stringify(jobs))
